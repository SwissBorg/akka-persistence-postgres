...
import akka.persistence.jdbc.journal.dao.Fakes.FakeEventTagConverter
...
class ByteArrayJournalSerializerTest extends SharedActorSystemTestSpec() {
  it should "serialize a serializable message and indicate whether or not the serialization succeeded" in {
    val serializer = new ByteArrayJournalSerializer(serialization, new FakeEventTagConverter(Map()))
...
// TODO do we need it?
object Fakes {
  class FakeEventTagConverter(tagMapping: Map[String, Int]) extends EventTagConverter {
    override def getIdByName(name: String): Future[Int] = tagMapping.get(name).fold[Future[Int]](Future.failed(new NoSuchElementException))(Future.successful)

    override def getIdByNameForce(name: String): Int = tagMapping.getOrElse(name, 0)
  }
}
