...
export DESTINATION_JOURNAL_TABLE_NAME='journal_flat'
...
psql -q ${CONNECTION_OPTIONS} --file="2-fill-event-tag.sql"
...
echo ""
echo "copy data"
psql -q ${CONNECTION_OPTIONS} --file="3-copy-data.sql"
...
psql -q ${CONNECTION_OPTIONS} --file="4-move-sequence.sql"
...
psql -q ${CONNECTION_OPTIONS} --file="5-create-indexes.sql"
...
#psql -q ${CONNECTION_OPTIONS} --command="SELECT nextval('journal_flat_ordering_seq');" # dont do it on production !!!
...
#psql -qt ${CONNECTION_OPTIONS} --command="INSERT INTO public.journal_flat(persistence_id, sequence_number, deleted, tags, message) VALUES ('pp-1', 1000001, true, '{}', '0x22');"

sleep 3
echo ""
echo "missing orderings in journal or journal_flat"
...
psql -q ${CONNECTION_OPTIONS} --file="6-drop-migration-procedures.sql"
