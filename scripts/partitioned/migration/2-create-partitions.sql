...
BEGIN
    FOR row IN
        ...
        LOOP
            ...
            FOR i IN 0..(row.max_sequence_number/table_size) LOOP
                    EXECUTE 'CREATE TABLE IF NOT EXISTS public.j_' || transformed_partition_id || '_' || i ||' PARTITION OF public.j_' || transformed_partition_id || ' FOR VALUES FROM (' || i*table_size || ') TO (' || (i+1)*table_size || ');';
            END LOOP ;
        END LOOP;
END ;
$$ LANGUAGE plpgsql;
